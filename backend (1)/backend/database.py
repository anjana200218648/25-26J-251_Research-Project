...
def _register_psycopg2_varchar_caster(dbapi_conn, connection_record):
    """
    Register psycopg2 type caster for varchar (OID 1043) on EVERY new
    connection. This is the correct fix for:
      sqlalchemy.exc.InvalidRequestError: Unknown PG numeric type: 1043
    The caster must be registered on the raw dbapi connection object,
    not globally, because psycopg2 connection pools create new connections
    that don't inherit global registrations reliably.
    """
    try:
        import psycopg2.extensions as _ext
        _caster = _ext.new_type((1043,), "VARCHAR", _ext.UNICODE)
        _ext.register_type(_caster, dbapi_conn)
    except Exception as _e:
        print(f"[WARN] psycopg2 varchar type caster registration failed: {_e}")
...
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "postgresql+psycopg2://postgres:123123123@localhost:5432/maindgard2"
)
...
class Complaint(Base):
    ...
    text = Column(Text, nullable=True)  # TEXT type එක හරි
    ...
    def risk_level(self, value):
        ...
        # risk_level එක set කරනකොට risk_score එක auto assign කරන්න
        if value == "High Risk":
            self.risk_score = 80
...
class User(Base):
    ...
    voice_enrolled = Column(String(8), nullable=True, default='false')  # 'true' or 'false'
    encrypted_speaker_embedding = Column(Text, nullable=True)
...
class VoiceSession(Base):
    ...
    encrypted_session_embedding = Column(Text)
    login_timestamp = Column(DateTime(timezone=True), server_default=func.now())
    login_phrase = Column(String(512))
    speaker_similarity = Column(Float)
    is_active = Column(Boolean, default=True)
    expires_at = Column(DateTime(timezone=True))


class VoiceComplaint(Base):
    ...
    speaker_verified = Column(Boolean)
    speaker_similarity = Column(Float)
...
class LoginAttempt(Base):
    ...
    success = Column(String(8), nullable=False)  # 'true' or 'false'
    attempt_type = Column(String(32), nullable=True)  # 'password', 'voice', 'both'
    speaker_similarity = Column(String(16), nullable=True)
    phrase_match = Column(String(8), nullable=True)
...
class ComplaintsRepo:
    ...
    def insert_one(self, doc: Dict[str, Any]):
        with self._Session() as s:
            ...
            s.add(c)
            s.flush()   # assigns c.id without committing
            inserted_id = c.id  # capture id before commit
            s.commit()
            # Do NOT call s.refresh(c) here — it re-queries and re-triggers
            # the OID 1043 type resolution bug in psycopg2/SQLAlchemy.
            # We already have the id from flush().
    ...
    def find(self, filters: Optional[Dict[str, Any]] = None, limit: Optional[int] = None) -> Iterable[Dict[str, Any]]:
        with self._Session() as s:
            ...
            if "risk_score" in f:
                q = q.filter(Complaint.risk_score == f["risk_sccore"]) 
            if "region" in f:
    ...
    def count_documents(self, filters: Optional[Dict[str, Any]] = None) -> int:
        with self._Session() as s:
            ...
            if 'user_id' in f:
                uid = f['user_id']
                # Match both string and integer representations in JSONB payload
                q = q.filter(
                    Complaint.payload.contains({'user_id': uid}) |
                    Complaint.payload.contains({'user_id': int(uid) if str(uid).isdigit() else uid})
                )
            if 'risk_level' in f:
                q = q.filter(Complaint.risk_level == f['risk_level'])
            return q.count()

    def update_text_report(self, complaint_id: Any, text_report: str, generated_at: datetime) -> bool:
        """Save a generated text report to an existing complaint record."""
        with self._Session() as s:
            try:
                cid = int(complaint_id)
                r = s.query(Complaint).filter(Complaint.id == cid).first()
            ...
            r.text_report = text_report
            r.report_generated_at = generated_at
            # Also mirror into payload so find() returns it too
            payload = dict(r.payload or {})
            payload['text_report'] = text_report
            payload['report_generated_at'] = generated_at.isoformat()
            r.payload = payload
            s.commit()
            return True
...
def init_db():
    ...
    try:
        from sqlalchemy import text as _sa_text, event as _sa_event
        engine = create_engine(
            DATABASE_URL,
            pool_pre_ping=True,
        )
        # Register varchar (OID 1043) type caster on EVERY new connection.
        # This is the definitive fix for: Unknown PG numeric type: 1043
        _sa_event.listen(engine, "connect", _register_psycopg2_varchar_caster)
        print("[INFO] psycopg2 varchar type caster registered on engine pool.")
        ...
        # ---- Safe column migration: add new columns if missing ----
        from sqlalchemy import text as sa_text
        with engine.connect() as conn:
            for col_name, col_ddl in [
                ("text_report",         "ALTER TABLE complaints ADD COLUMN IF NOT EXISTS text_report TEXT"),
                ("report_generated_at", "ALTER TABLE complaints ADD COLUMN IF NOT EXISTS report_generated_at TIMESTAMPTZ"),
            ]:
                try:
                    conn.execute(sa_text(col_ddl))
                    conn.commit()
                except Exception as col_err:
                    print(f"[WARN] Column migration '{col_name}': {col_err}")
