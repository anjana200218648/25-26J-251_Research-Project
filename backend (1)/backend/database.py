...
DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "postgresql+psycopg2://postgres:admin@localhost:5432/safekid"
)
...
class Complaint(Base):
    ...
    # Generated text report saved by the admin
...
class User(Base):
    ...
    role = Column(String(32), nullable=True)  # 'father', 'mother', 'guardian'
...
class VoiceComplaint(Base):
    ...
    guardian_name = Column(String(256), nullable=False)
    child_name = Column(String(256), nullable=False, index=True)
    age = Column(Integer, nullable=False)
    phone_number = Column(String(32), nullable=False)
    region = Column(String(256))
    
    # Complaint Details
    complaint = Column(Text, nullable=False)  # Transcribed text
    child_gender = Column(String(32))
    hours_per_day_on_social_media = Column(Float)
    reporter_role = Column(String(64))
    device_type = Column(String(64))
    
    # Voice-specific metadata
    speaker_verified = Column(Boolean, default=False)
    speaker_similarity = Column(Float)
    audio_duration = Column(Float)
    voice_confidence = Column(Float)  # Transcription confidence
    ...
    temporal_drift_score = Column(Float)
    risk_explanation = Column(Text)
    triggered_indicators = Column(Text)  # JSON string
    temporal_data = Column(Text)  # JSON string
...
class UsersRepo:
    ...
    def create_user(self, name: str, email: str, phone: str, password_hashed: str, role: str = None) -> Dict[str, Any]:
        with self._Session() as s:
            existing = s.query(User).filter(User.email == email).first()
            if existing:
                raise ValueError("User with this email already exists")
            u = User(name=name, email=email, phone=phone, password=password_hashed, role=role)
            s.add(u)
            s.commit()
            s.refresh(u)
            return {"_id": str(u.id), "name": u.name, "email": u.email, "phone": u.phone, "role": u.role}
    ...
    try:
        ...
        with engine.connect() as conn:
            for col_name, col_ddl in [
                ...
                ("role",                "ALTER TABLE users ADD COLUMN IF NOT EXISTS role VARCHAR(32)"),
            ]:
                try:
