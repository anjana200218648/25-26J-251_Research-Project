...
def verify_otp():
    ...
    try:
        ...
        if datetime.now(timezone.utc).replace(tzinfo=None) > stored_data['expires_at']:
        ...
        traceback.print_exc()
        return jsonify({'message': 'Verification failed', 'detail': str(e)}), 500
...
def forgot_password():
    ...
    try:
        ...
        traceback.print_exc()
        return jsonify({'message': 'Failed to process request', 'detail': str(e)}), 500
...
def verify_reset_otp():
    ...
    try:
        ...
        print(f"[ERROR] Verify reset OTP failed: {e}")
        traceback.print_exc()
        return jsonify({'message': 'Verification failed', 'detail': str(e)}), 500
...
def reset_password():
    ...
    try:
        ...
        if datetime.now(timezone.utc).replace(tzinfo=None) > token_data['expires_at']:
        ...
        traceback.print_exc()
        return jsonify({'message': 'Failed to reset password', 'detail': str(e)}), 500
...
def resend_otp():
    ...
    try:
        ...
        otp_storage[email]['expires_at'] = datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(minutes=5)
        ...
        traceback.print_exc()
        return jsonify({'message': 'Failed to resend OTP', 'detail': str(e)}), 500
