...
class User:
    ...
    @staticmethod
    def create_user(name, email, phone, password, role=None):
        users_repo = get_users_repo()
        hashed_password = generate_password_hash(password)
        user = users_repo.create_user(name=name, email=email, phone=phone, password_hashed=hashed_password, role=role)
        return user
