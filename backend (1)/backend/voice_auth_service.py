            ...
            if WHISPER_LOCAL_MODEL_PATH and os.path.exists(str(WHISPER_LOCAL_MODEL_PATH)):
                try:
                    ...
                    def _quiet_stdio_local():
                        """Windows-safe stdout/stderr suppression."""
                    ...
                    with _quiet_stdio_local():
                        self.whisper_model = whisper.load_model(str(WHISPER_LOCAL_MODEL_PATH))
            ...
            if not self.whisper_model:
                ...
                for m in models_to_try:
                    try:
                        ...
                        # Suppress progress output during model download/load (Windows-safe)
                        from contextlib import contextmanager
                        import sys as _sys_mod
                        import io
                        ...
                        with _quiet_stdio_local():
                            self.whisper_model = whisper.load_model(m)
                        print(f"[INFO] Whisper '{m}' model initialized successfully")
                        break
                    except Exception as e:
                        print(f"[WARN] Failed to initialize Whisper '{m}': {e}")
                        tried.append(m)
