...
const generatePDFReport = (result: ComplaintResult) => {
  ...
  doc.text('SafeKid Scan Risk Assessment Report', 105, 15, { align: 'center' });
  doc.setFontSize(12);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text('SafeKid Scan Risk Assessment System | Confidential Document', 105, 285, { align: 'center' });
...
<div class="sw">
  <div class="header">
    <h1>SafeKid Scan අවදානම් තක්සේරු වාර්තාව</h1>
  ...
  <div class="footer">
    SafeKid Scan අවදානම් තක්සේරු පද්ධතිය | රහස්‍ය ලේඛනය
  </div>
</div>`;
...
const ComplaintResult = () => {
  ...
  const { t } = useLanguage();
  const cr = t?.complaintResult || {} as any;
  const cf = t?.complaintForm || {} as any;
  const result: ComplaintResult | undefined = location.state?.result;

  const translateDeviceType = (deviceType?: string) => {
    if (!deviceType) return '';
    const dt = deviceType.toLowerCase();
    if (dt === 'mobile' || dt === 'smartphone') return cf.mobile || cf.smartphone || 'Mobile';
    if (dt === 'tablet') return cf.tablet || 'Tablet';
    if (dt === 'laptop') return cf.laptop || 'Laptop';
    if (dt === 'desktop') return cf.desktop || 'Desktop';
    return deviceType;
  };
  ...
  const temporalPatternLabel = (() => {
    switch (temporalPattern) {
      case 'critical_escalation':
        return cr.patternCritical;
      case 'escalation':
        return cr.patternEscalation;
      case 'stable_high':
        return cr.patternStableHigh;
      case 'stable':
        return cr.patternStable;
      case 'improving':
        return cr.patternImproving;
      default:
        return cr.patternBaseline;
    }
  })();
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="container mx-auto px-4 py-8 max-w-4xl">
          <div className="text-center py-12">
            ...
            <h2 className="text-xl font-semibold text-gray-700 mb-2">{cr.analyzingTitle}</h2>
            <p className="text-gray-600">{cr.analyzingDesc}</p>
          </div>
        </main>
      </div>
    );
        ...
        <main className="container mx-auto px-4 py-8 max-w-4xl">
          <div className="text-center space-y-8 py-12">
            ...
            <div className="space-y-4">
              <h2 className="text-3xl font-bold text-gray-900">{cr.noResultTitle}</h2>
        ...
        <div className="space-y-8">
          ...
          <div className="text-center space-y-4">
            ...
            <p className="text-lg text-gray-600 font-medium">
              {cr.pageSubtitle} {displayResult.child_name}
            </p>
          </div>
            ...
            <div className="relative z-10 flex flex-col items-center space-y-8">
              ...
              <div className="text-center space-y-3 max-w-md">
                <h2 className={`text-3xl font-bold ${getRiskColor(riskLevel)}`}>
                  {riskLevel === 'high' ? cr.riskHigh : riskLevel === 'medium' ? cr.riskMedium : cr.riskLow}
                </h2>
                ...
                <Badge variant="secondary" className={`px-4 py-2 text-sm font-medium ${getRiskBgColor(riskLevel)} ${getRiskColor(riskLevel)} border-0`}>
                  ...
                  {cr.riskScoreLabel} {riskScore}/100
                </Badge>
              </div>
            </div>
          </Card>
            ...
            <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm hover:shadow-xl transition-all duration-300">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-gray-900">{cr.childGuardianSection}</h3>
                    <p className="text-sm text-gray-600">{cr.childGuardianSubtitle}</p>
                  </div>
                </div>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
                    <div className="flex items-start gap-4 p-4 rounded-xl border bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-100">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-900">{cr.childName}</p>
                        <p className="text-lg font-semibold text-blue-700">{displayResult.child_name}</p>
                        <p className="text-xs text-gray-500 mt-1">{displayResult.child_gender === 'M' || displayResult.child_gender === 'Male' ? cr.male : cr.female}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-4 p-4 rounded-xl border bg-gradient-to-r from-purple-50 to-pink-50 border-purple-100">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-900">{cr.age}</p>
                        <p className="text-lg font-semibold text-purple-700">{displayResult.age} {cr.yearsOld}</p>
                      </div>
                    </div>

                    <div className="flex items-start gap-4 p-4 rounded-xl border bg-gradient-to-r from-indigo-50 to-cyan-50 border-indigo-100">
                      <div className="rounded-full bg-indigo-100 p-3">
                        <Smartphone className="h-5 w-5 text-indigo-600" />
                      </div>
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-900">{cr.deviceType || 'Device'}</p>
                        <p className="text-lg font-semibold text-indigo-700 capitalize">{translateDeviceType(displayResult.device_type)}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-4 p-4 rounded-xl border bg-gradient-to-r from-green-50 to-emerald-50 border-green-100">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-900">{cr.guardian}</p>
                    ...
                    <div className="flex items-start gap-4 p-4 rounded-xl border bg-gradient-to-r from-orange-50 to-red-50 border-orange-100">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-900">{cr.contact}</p>
            ...
            <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm hover:shadow-xl transition-all duration-300">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-gray-900">{cr.usageSection}</h3>
                    <p className="text-sm text-gray-600">{cr.usageSubtitle}</p>
                  </div>
                </div>
                ...
                <div className="px-6 pb-6">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className={`flex items-start gap-4 p-4 rounded-xl border ${
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-900">{cr.dailyUsage}</p>
                        ...
                        {displayResult.hours_per_day_on_social_media} {cr.hoursPerDay}</p>
                        ...
                        <div className="flex items-center justify-between mt-2">
                          <p className="text-xs text-gray-500">{cr.recommended}</p>
                          {displayResult.hours_per_day_on_social_media >= 8 && (
            ...
            <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm hover:shadow-xl transition-all duration-300">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-gray-900">{cr.temporalSection}</h3>
                    <p className="text-sm text-gray-600">{cr.temporalSubtitle}</p>
                  </div>
                </div>
                  ...
                  <div className="rounded-xl border border-blue-100 bg-gradient-to-r from-blue-50 to-sky-50 p-5 space-y-5">
                    {!hasTemporalData ? (
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-blue-800">{cr.noHistory}</p>
                          <p className="text-sm text-blue-700">{cr.noHistoryDesc}</p>
                        </div>
                      </div>
                    ) : (
                      <>
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          <Badge className={`border ${temporalDriftScore >= 0 ? 'bg-rose-100 text-rose-700 border-rose-200' : 'bg-emerald-100 text-emerald-700 border-emerald-200'}`}>
                            {temporalDriftScore >= 0 ? '+' : ''}{temporalDriftScore} {cr.temporalPoints}
                          </Badge>
                        </div>
            ...
            <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm hover:shadow-xl transition-all duration-300">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-gray-900">{cr.complaintSection}</h3>
                    <p className="text-sm text-gray-600">{cr.complaintSubtitle}</p>
                  </div>
                </div>
                ...
                <div className="px-6 pb-6">
                  <div className="p-6 rounded-xl border bg-gradient-to-r from-amber-50 to-orange-50 border-amber-100">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-900 mb-3">{cr.concernDetails}</p>
                        ...
                        <div className="mt-4 flex items-center gap-4 text-sm text-gray-500">
                          <div className="flex items-center gap-2">
                            ...
                            <span>{cr.submitted} {new Date(displayResult.timestamp).toLocaleDateString()}</span>
                          </div>
                          <div className="flex items-center gap-2">
            ...
            <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm hover:shadow-xl transition-all duration-300">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-gray-900">{cr.recommendationsSection}</h3>
                    <p className="text-sm text-gray-600">{cr.recommendationsSubtitle}</p>
                  </div>
                </div>
                        ...
                        {riskLevel === 'high' ? (
                          <>
                            <li className="flex items-start gap-3">
                              ...
                              <span><strong>{cr.recHighTitle1}</strong> {cr.recHighBody1}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-red-500 font-bold">•</span>
                              <span><strong>{cr.recHighTitle2}</strong> {cr.recHighBody2}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-red-500 font-bold">•</span>
                              <span><strong>{cr.recHighTitle3}</strong> {cr.recHighBody3}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-red-500 font-bold">•</span>
                              <span><strong>{cr.recHighTitle4}</strong> {cr.recHighBody4}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-red-500 font-bold">•</span>
                              <span><strong>{cr.recHighTitle5}</strong> {cr.recHighBody5}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-red-500 font-bold">•</span>
                              <span><strong>{cr.recHighTitle6}</strong> {cr.recHighBody6}</span>
                            </li>
                          </>
                        ) : riskLevel === 'medium' ? (
                          <>
                            <li className="flex items-start gap-3">
                              ...
                              <span><strong>{cr.recMedTitle1}</strong> {cr.recMedBody1}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-amber-500 font-bold">•</span>
                              <span><strong>{cr.recMedTitle2}</strong> {cr.recMedBody2}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-amber-500 font-bold">•</span>
                              <span><strong>{cr.recMedTitle3}</strong> {cr.recMedBody3}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-amber-500 font-bold">•</span>
                              <span><strong>{cr.recMedTitle4}</strong> {cr.recMedBody4}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-amber-500 font-bold">•</span>
                              <span><strong>{cr.recMedTitle5}</strong> {cr.recMedBody5}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-amber-500 font-bold">•</span>
                              <span><strong>{cr.recMedTitle6}</strong> {cr.recMedBody6}</span>
                            </li>
                          </>
                        ) : (
                          <>
                            <li className="flex items-start gap-3">
                              ...
                              <span><strong>{cr.recLowTitle1}</strong> {cr.recLowBody1}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-green-500 font-bold">•</span>
                              <span><strong>{cr.recLowTitle2}</strong> {cr.recLowBody2}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-green-500 font-bold">•</span>
                              <span><strong>{cr.recLowTitle3}</strong> {cr.recLowBody3}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-green-500 font-bold">•</span>
                              <span><strong>{cr.recLowTitle4}</strong> {cr.recLowBody4}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-green-500 font-bold">•</span>
                              <span><strong>{cr.recLowTitle5}</strong> {cr.recLowBody5}</span>
                            </li>
                            <li className="flex items-start gap-3">
                              <span className="text-green-500 font-bold">•</span>
                              <span><strong>{cr.recLowTitle6}</strong> {cr.recLowBody6}</span>
                            </li>
                          </>
                        )}
                      </ul>
          ...
          <div className="text-center py-8 border-t border-gray-200">
            <div className="flex items-center justify-center gap-2 mb-4">
              ...
              <span className="text-lg font-semibold text-gray-900">{cr.footerTitle}</span>
            </div>
